...
11. **正方第 1 輪的 prompt 缺結構，產出品質明顯低於第 2 輪**（2026-07-29 隊員回報，
    vic 複查確認）：隊員反映第 2 輪論證的排版明顯比第 1 輪好。查下來不是錯覺，
...
12. **反方在結構上系統性強於正方**（2026-07-29 隊員回報，vic 複查）：隊員反映
    ...
    **早於修正 14 小時**。
    ...
    - **③ 裁判守則單向傾斜 ← 最實質**：守則 1 只保護反方（「不可略過批評直接採信正方」），
    ...
    - **⑥ 版面音量 2:1（Alan 範圍）**：`report/view_builder.py:282-284` 與
      `report/builder.py:92-100` 給反方兩個區塊（`bear_critique` + `bear_argument`）、
      正方一個。**讀者「感覺反方較強」有一部分純粹是字數造成的**，與論證品質無關。

    **撤回一項**：`debate_adjustment` 的 −15～+5 看似偏向反方，但 prompt 明講那是
    「對這份分析報告本身的信心調整，不是對市場的看多看空」，屬 ADR-5 刻意的防灌分
    設計，與正反方無關。它真正對應的問題已併入 ③。

    **建議順序**：先確認隊員看的版本 → 若仍成立，優先修 ③（補一句對等保護，改動一行）
    → 再考慮 ②（要給正方第 2 輪加 `critique` 欄位，動到 debate 契約與下游，較大）。
