...
> 最後更新：2026-08-02（v1.2-final ＋ vic 裁判機制改版整合進 v3）
...
14. **裁判機制實測與 15 分鐘上限硬保證（2026-08-01 完成，見 `JUDGE_TEST_REPORT.md`
    ...
    **上方待辦 9／12 的「只有結構推論、沒有輸出實證」諸項已用真實輸出回答**
    ...
    - 9「第 2 輪正方是真反駁還是換句話說」：**是真反駁**（字元 bigram 重疊 0.19–0.23，
      引用組合有實質增減）。
      ⚠️ 待辦 9① 原文「可用 `agent/filters/content.py` 既有的 Jaccard」有兩個錯：
      函式在 `agent/filters/dedup.py:31` 不在 `content.py`，**而且它不能用**——
      以空白切 token，中文整段切不開，同一組文字給出 0.021/0.030/0.100 純噪音。
    - 12①②④⑥ **有優勢但不壓倒**：15 條裁判摘要人工歸類為反方 8／正方 5／打平 2。
      **因此建議順序裡的 ② （給正方 `critique` 欄位、動 debate 契約）本次資料不支持**，
      53:33 的落差不足以支撐破壞性變更。
    - 13 **確認成立**（見下條，仍未修）。

    **本次新發現且已修**：`debate_adjustment` 在真實區間內是常數（改為逐點判定加總）、
    Step D 是四步裡唯一拿不到權重表卻被要求依權重評斷的、信心分數與等級是兩套
    獨立系統無一致性檢查、正方過度承認批評（改寫反駁輪任務並加共用判定標準）、
    單次 Bedrock 卡頓可讓整跑達 1189s（雙層時間保證）。

    **仍未驗證的路徑**：fallback（無辯論）、`bull_failed`／`bear_failed`、
    `step_c_inference_fallback`、真實 `ReadTimeoutError`——需強制失敗的 run。

15. **`DEBATE_ROUND_TIME_FACTOR` 的校準基礎不乾淨（2026-08-01）**：目前值 2.2 是拿
    「第 1 輪計時漏算正方」時期的分母算出來的（1.68/1.91/2.09）。該 bug 已於
    `e8e3020` 修掉，但係數沒有回頭重算——它比舊值 1.5 保守，方向正確，
    等累積足夠的乾淨樣本應重新校準。
