...
| 幣/鏈 | 5 年歷史 | 怎麼拿 |
|---|---|---|
| BTC | ✅ 已解決 | `blockchain.info` 免費 charts API。hashrate／24h 交易筆數有 1822 天（2021-07-21~2026-07-19）；mempool 筆數/大小只有 1497 天（~4年）。已寫成 `pipeline/fetch_onchain_history.py`，輸出 `raw_data/onchain/BTC/onchain_history_daily.csv`，實測跑出 1824 天（2021-07-22~2026-07-20），細節見 `流程紀錄.md` Step 23 |
| ETH | ✅ 已解決 | `etherscan.io/chart/gasprice?output=csv`，免 key 公開圖表下載端點，回溯到 2015 年。已寫成 `pipeline/fetch_onchain_history.py`，輸出 `raw_data/onchain/ETH/onchain_history_daily.csv`，實測跑出 4009 天（2015-07-30~2026-07-20），細節見 `流程紀錄.md` Step 23 |
