...
- [ ] **9.7** 全套測試 + 一次真實 Bedrock 驗證（**題目要用非 medium 主視野**）
  ...
  - **同場加驗 `debate_adjustment` 能不能為正**（決策⑧ 的收尾）：`af3cd3e` 的雙向
    裁判守則與 vic 的裁判篇幅條款**都只有字串存在性測試**，正值實際會不會出現無人驗過。
    四次真實跑是 −10／−12／−6／−10 全負，那個實證不能只用單元測試回應。
    驗收＝至少出現一次正的 `debate_adjustment`，**或**記錄為何仍全負（該次批評是否
    真的都成立），兩者皆可結案，但不可略過不看。
    ⚠ **反向也要看**：本輪一次上了兩個未經真實驗證的偏正方調整（`af3cd3e` 的上調
    觸發條件＋裁判篇幅條款），STATUS.md 12 自己就警告過「從相反方向把同一件事修兩次，
    變成正方過強」。所以同時確認調整值沒有翻成系統性為正、反方的成立批評仍有下修
    ——只檢查「有沒有正值」的驗收會在正方過強時照樣通過
  - _Requirements: R8-6_
