...
| # | 議題 | 裁定 | 影響本規格之處 |
|---|---|---|---|
| ⑦ | 待辦 11（正方第1輪缺結構） | 資料不支持（兩輪皆 6 點條列）。**不加任務骨架**，只補長度上限與刪除誘導散文的措辭 | `DEBATE_LENGTH_RULE` 三處共用 |
| ⑧ | 待辦 12（反方系統性強於正方） | 主因已由 vic 修掉；**另發現裁判守則單向**（反方打中必下修、正方擋住無上修），四次真實跑 debate_adjustment 全負。已補雙向守則 | Step D 裁判守則第1條、debate_adjustment 說明 |
| ⑨ | 報告格式：裁判快速整理辯論重點 | 新增 `debate_summary` 欄位（3-5 點＋evidence id），置於執行摘要後、結論前；完整逐字稿改預設收合（`<details markdown="1">`），無摘要時不收合 | Step D 新輸出欄位；`report/builder.py` 新章節 |
