...
6. **角色不是綁死在特定帶上**（R7-3，2026-07-28 起）。
   主視野由題目的時間範圍動態決定，角色依此推導：
   ...
   判斷請用 `is_current_signal(h, primary)`，**不要**直接比對
   `CURRENT_SIGNAL_HORIZONS` 常數集合（那只是主視野=`medium` 時的預設分組，
   保留作相容性）。題目問「最近一年」時，`long`／`structural` 會變成當前訊號。
