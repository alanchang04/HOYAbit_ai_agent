...
1. 🔴 **用新機制重跑 Q2／Q3**——逐點判定與判定標準只在 Q1（多源整合）跑過，
   §9.2 那張「三題散開成 −4／−7／−9」是人工歸類**試算**，不是模型實吐的 verdict。
   **假設驗證題與比較分析題從未真的跑過新 schema。** 零程式風險，先做這個。
2. 🔴 **分類器加幣種數量覆寫**：題目出現 2 個以上幣種池成員 → 直接判 comparison。
   現況「SOL 與 XRP 何者風險較高」會判成 multi_source，導致 `coin2` 偵測不執行、
   **拿單一幣種證據回答雙幣種題目**（錯在蒐集階段，裁判救不回來）。
   `detect_coins_in_text()` 已現成。
3. 🟠 比較題的 verdict 級距改對稱——現況 `bear_valid=−3`/`bull_defended=+1` 的
   不對稱理由是「反方＝質疑者」，但比較題兩邊都是倡議者。實測同一份市場事實下，
   主幣寫 SOL 是 −9、寫 XRP 是 −1，**8 分落差純粹來自題目語序**。
