...
- 比賽當日環境（網路、機器、AWS 憑證形式）：PDF 未描述，僅知「主辦方環境問題可重跑一次」。**2026-07-24 追加**：Alan 原本假設「當天只要把 AWS 系統換成主辦方帳號即可」，但這個假設**沒有規則依據，PDF 完全沒提到會提供官方帳號**，需盡快向主辦方確認。若確認真的要換帳號，這件事本身有風險——這次自己的帳號一路踩了 Gemini 專案配額、Bedrock model access 被拒/legacy 限制、IAM 權限、App Runner 帳號限制等好幾個帳號層級的坑，換新帳號不保證不會遇到類似或不同的問題，**建議至少提前一天用主辦方帳號實際跑一次「憑證→Bedrock model access→完整 pipeline」，不要等到當天才第一次測**。
...
|---|---|---|---|---|---|---|
| 1 | 多源資料整合（5 類） | 🟡 | 5 collectors 實測可跑，含真實 Bedrock 端到端驗證 | social 受 Reddit 403 地理封鎖（本機/雲端 IP 皆會被擋，賽場網路實測未知）；news 目前為每幣官方源（含 BNB/XRP HTML 解析退階），改版會失效需賽前重跑確認 | P1（來源穩定性） | 賽前用實際比賽網路環境重跑一次 |
| 2 | 分層推理（事實→推論→結論） | ✅ | 四步鏈＋多輪辯論（正反方最多 2 輪，反方自報收斂），**已用 Bedrock 真實驗證 3 種題型** | — | — | 已完成 |
| 3 | 不確定性與限制說明 | ✅ | 信心等級＋限制＋可推翻條件，實測輸出具體 | — | — | 抽查報告 |
| 4 | 證據可回溯 | ✅ | schema 含全必要欄位＋query 參數；報告 id 強制檢查；macro 重複證據 bug 已修（Phase 0.4） | 無原文快照/雜湊（P2，非必要） | P2 | — |
| 5 | 洞察角度 | ✅ | 辯論交鋒、技術指標、跨來源矛盾偵測、雙幣相對指標（相關係數/beta/最大回撤） | 衍生品構想（Ken）仍是獨立 prototype 腳本，未接進 orchestrator | P2 | 衍生品 collector 化 |
| 6 | 報告三必要章節 | ✅ | report.md 六節結構（新增執行摘要）＋長文 markdown 排版 | — | — | — |
| 7 | Evidence List 格式 | ✅ | evidence.json 符合 schema | — | — | — |
| 8 | Execution Log 格式 | ✅ | jsonl 含時間戳/工具/狀態；四面板可視化升級已完成 | — | — | — |
| 9 | 15 分鐘/一次執行穩定性 | ✅ | **真實 Bedrock 實測**：多源整合 309.6s／假設驗證 178.7s／比較分析 357.4s，皆在 900s 預算內；隔離/備援/degraded 有測試 | 賽場實際網路環境未知（同 #1） | P1 | — |
| 10 | 不用第三方完整判斷 | ✅ | 全部原始數據＋自有推理鏈 | — | — | — |
| 11 | LLM 僅 Bedrock | ✅ | **2026-07-24 完整打通**：region `ap-northeast-1`、Claude Sonnet 4.5，`.env` 已設定為 `LLM_BACKEND=bedrock` | — | — | 已完成 |
| 12 | Live Demo 部署網址 | 🟡 | Docker image 已 build 並 push 到 ECR；App Runner 所需 IAM role 已建立 | **建立 App Runner service 被 AWS 帳號「Free plan」限制擋住**（`SubscriptionRequiredException`），需完成帳號驗證/升級 | **P0** | 帳號解鎖後跑 `aws apprunner create-service` |
| 13 | 現場執行錄影 | ❌ | — | 未錄 | P0（賽前） | 影片連結 |
| 14 | 簡報＋AWS 架構圖（圖檔） | 🟡 | PITCH_REFERENCE.md 素材＋ASCII 圖 | 需畫正式圖、做簡報 | P0（賽前） | 簡報成品 |
| 15 | AWS Kiro（+10%） | 🟡 | `.kiro/` 下 12 個檔案：specs 三件套（requirements/design/tasks/team-division）、4 份 steering、2 個 hooks、MCP 設定，長期實際使用（非補拍證據） | 需整理成簡報展示素材 | P1 | 簡報截圖 |
| 16 | 付費資料揭露 | ✅ | 全免費來源，無任何付費 API | — | — | — |
| 17 | 報告語言（繁中） | ✅ | 實測輸出繁中、引用保留英文原文 | — | — | — |

**比賽必要缺口總結（P0，2026-07-24 更新）**：僅剩 **實際部署**（#12，卡在 AWS 帳號限制，非技術問題）、**錄影**（#13）、**簡報**（#14）——技術面（分層推理、Bedrock、穩定性、可回溯性）已全數驗證完成。
**已解決（曾是缺口）**：Bedrock 實測（#11）、15 分鐘穩定性（#9）、macro 重複（#4）、AWS Kiro 加分素材大致齊備（#15，只差整理成簡報）。
**展示/加分（P1/P2）**：衍生品 collector 化、Reddit/news 賽場網路環境重驗、原文快照雜湊（非必要）。
